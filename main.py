...
qam_combo.setCurrentIndex(_qam_options.index(QAM_N))
...
def update():
    ...
    payload = b"The quick brown fox jumped over the lazy hare"
    sdr_tx.transmit(tx_pipe.encode(payload))
    ...
    snippets = rx_pipe.sync.find_frames(sig_rx)
    if snippets:
        import numpy as _np
        freq = _np.fft.fft(_np.array(snippets))
        active = _np.array([
            rx_pipe.modulator.demodulate(freq[i]) for i in range(len(snippets))
        ])
        data_syms = rx_pipe.equalizer.equalize(active)
        symbol = _np.mean(data_syms, axis=0)

        curve_sym_real.setData(symbol.real)
        curve_sym_imag.setData(symbol.imag)
        scatter.setData(symbol.real, symbol.imag)

        result = rx_pipe.decode_averaged(sig_rx)
        if result is not None:
            print(result.decode("utf8", errors="ignore"))
